    ...
    void run(int port, String id, String ip){
        ...
        Server.get("/view/:table", (request, response)->{
            String html = "<!DOCTYPE html>\n" +
                    ...
                    "</html>";
            StringBuilder tableData = new StringBuilder();
            if(tableTableRow.get(request.params("table"))==null){
                response.status(404, "No such table");
            }
            tableData.append("<table>");
            for(String rowName: tableTableRow.get(request.params("table")).keySet()){
                tableData.append(String.format("<tr><td>%1$s</td>", rowName));
                Row row = tableTableRow.get(request.params("table")).get(rowName);
